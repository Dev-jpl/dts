...
<template>
  <div>
    ...
    <div class="flex justify-end px-2 pt-1 md:hidden border-b border-gray-200 pb-1">
      <button type="button" @click="isCollapsed = !isCollapsed"
        class="text-xs text-gray-500 flex items-center gap-1 py-1">
        <span>{{ isCollapsed ? 'Show Steps' : 'Hide Steps' }}</span>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
          class="size-3 transition-transform duration-200" :class="isCollapsed ? '-rotate-90' : 'rotate-90'">
    ...
    <div :class="isCollapsed ? 'hidden' : ''"
      class="md:block grid grid-cols-4 md:divide-y md:grid-cols-1 md:grid-flow-row">
      <button
        type="button"
        v-for="step in steps"
        :key="step.title"
