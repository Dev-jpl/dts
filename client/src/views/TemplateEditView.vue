...
// Flag to skip the routing type watcher during initial load
...
function mapTemplateRecipientsToOfficeRefs(
  ...
  for (const tr of templateRecipients) {
    const officeMatch = allOffices.find((o) => o.return_value.office_code === tr.office_id);
    const recipient: Recipient = officeMatch
      ? { ...officeMatch.return_value, sequence: tr.sequence }
      : {
          office_code: tr.office_id,
          office: tr.office_name,
          region: "",
          service: "",
          sequence: tr.sequence,
        };

    if (tr.recipient_type === "cc") cc.push(recipient);
    else if (tr.recipient_type === "bcc") bcc.push(recipient);
...
function mapTemplateSignatoriesToItems(
  templateSignatories: TemplateSignatory[],
  allSignatories: any[]
): SignatoryItem[] {
  return templateSignatories.map((ts) => {
    const match = allSignatories.find((s) => s.return_value?.id === ts.signatory_id);
    if (match) {
      return match.return_value;
    }
    // Fallback: create item from template data
    return {
      id: ts.signatory_id ?? "",
      name: ts.name,
    ...
    const seqA = templateSignatories.find((t) => t.signatory_id === a.id)?.sequence ?? 0;
    const seqB = templateSignatories.find((t) => t.signatory_id === b.id)?.sequence ?? 0;
    return seqA - seqB;
  });
}
...
onMounted(async () => {
  ...
  try {
    ...
    form.urgency_level = tpl.urgency_level;
    form.remarks_template = tpl.remarks_template ?? "";
    originType.value = tpl.origin_type ?? "";
    routingType.value = tpl.routing_type ?? "";

    // Sender fields
    form.sender = tpl.sender ?? "";
    form.sender_position = tpl.sender_position ?? "";
    form.sender_office = tpl.sender_office ?? "";
    form.sender_email = tpl.sender_email ?? "";

    // Reverse-map document type: find matching library item
    if (tpl.document_type) {
      const items = documentTypeLibrary.data ?? [];
      const match = items.find((i) => i.return_value?.type === tpl.document_type);
      if (match) {
        selectedDocType.value = match.return_value;
      } else {
        // Fallback: create a synthetic value so the form shows something
        selectedDocType.value = { id: "", code: "", type: tpl.document_type };
      }
    }

    // Reverse-map action type
    if (tpl.action_type) {
      const items = actionLibrary.data ?? [];
      const match = items.find((i) => i.return_value?.action === tpl.action_type);
      if (match) {
        selectedAction.value = match.return_value;
      } else {
        selectedAction.value = { id: "", action: tpl.action_type };
      }
    }

    // Map recipients into separate lists
    const allOffices = officeLibrary.office.value ?? [];
    const { defaults, cc, bcc } = mapTemplateRecipientsToOfficeRefs(tpl.recipients, allOffices);
    ...
    signatories.value = mapTemplateSignatoriesToItems(tpl.signatories ?? [], allSignatories);

    // Auto-expand CC/BCC if they have recipients
    if (cc.length > 0) toggleRecipientCC.on();
    if (bcc.length > 0) toggleRecipientBCC.on();

    // Allow routing type watcher to function normally now
    isInitialLoad.value = false;
  } catch {
    error.value = "Template not found or not accessible.";
  } finally {
...
<template>
  <div class="flex-1 w-full h-full bg-gray-50">
    <!-- Header -->
    <div class="flex items-center justify-between p-2 gap-3 flex-wrap">
      <div class="flex items-center gap-2">
        ...
        <h1 class="text-sm font-semibold text-gray-800">Edit Template</h1>
      </div>
      <div class="flex gap-2 ml-auto shrink-0">
        <BaseButton :btn-text="'Cancel'" :action="() => router.push({ name: 'my-templates' })"
          :className="'flex items-center'" />
        <BaseButton :btn-text="'Save Changes'" :action="() => openSaveModal()" :disabled="isLoading"
          :className="'flex items-center'">
    ...
    <div v-else
      ...
      <div class="flex flex-col justify-start h-full p-6 sm:col-span-7 overflow-y-auto">
      ...
      <div class="flex flex-col justify-start h-full p-6 sm:col-span-5 overflow-y-auto">
    ...
    >
      <div class="w-full mt-3">
        <p class="text-sm text-center text-gray-500">Do you want to save these template changes?</p>
      </div>
    </ModalDialog>
