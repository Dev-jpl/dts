...
const confirmDeleteDraft = async () => {
  ...
  deleteError.value = '';
  try {
    await deleteDraft(transaction.value!.transaction_no);
    showDeleteModal.value = false;
    toast.success("Draft Deleted", "The draft document has been permanently deleted.");
  ...
  <div class="flex-1 w-full h-[calc(100svh-50px)] bg-white">
    ...
    <div class="flex items-center h-[50px] justify-between p-2 border-t border-gray-200 bg-gray-50">
      <div class="flex items-center">
        ...
        <div v-if="transaction"
          ...
          <div v-if="canRelease">
            <ReleaseModal :isOpen="modalReleaseOpen" :toggleReleaseModal="toggleReleaseModal"
              :trxNo="transaction.transaction_no" :transaction="transaction" @released="onDocumentReleased" />
          ...
          <div v-if="canMarkAsDone">
            <MarkAsDoneModal :isOpen="modalDoneOpen" :toggleModal="toggleDoneModal"
              :trxNo="transaction.transaction_no" :actionType="transaction.action_type" @done="onMarkedAsDone" />
        ...
        <!-- DELETE DRAFT -->
        <button v-if="isDraft && isOriginator" type="button" @click="showDeleteModal = true"
          class="flex items-center px-3 py-1.5 ml-3 text-xs text-red-600 border border-red-300 rounded-md hover:bg-red-50">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
            stroke="currentColor" class="mr-1.5 size-4">
            <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div class="bg-white rounded-xl shadow-xl w-full max-w-sm p-5 space-y-4">
        <h3 class="text-base font-semibold text-gray-800">Delete Draft</h3>
        <p class="text-sm text-gray-600">
          Are you sure you want to delete this draft document? This action
          <strong>cannot be undone</strong> and will permanently remove the document,
          its recipients, attachments, and all related data.
        </p>
        ...
        <div class="flex justify-end gap-3">
          ...
          <button type="button" :disabled="isDeleting" @click="confirmDeleteDraft"
